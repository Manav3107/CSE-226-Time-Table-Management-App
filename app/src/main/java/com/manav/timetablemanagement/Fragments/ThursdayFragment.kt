...
class ThursdayFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.getAllTimetables(requireContext(), "Thursday")?.observe(viewLifecycleOwner, Observer {
            timeTableAdapter.setData(it as ArrayList<Timetable>)
        })

        // Set up the FAB to open the dialog
        view.findViewById<FloatingActionButton>(R.id.fabAddEntryThursday).setOnClickListener {
            showAddDataDialog()
        }
        return view
    }
