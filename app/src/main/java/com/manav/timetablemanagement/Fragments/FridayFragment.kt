...
class FridayFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.getAllTimetables(requireContext(), "Friday")?.observe(viewLifecycleOwner, Observer {
            timeTableAdapter.setData(it as ArrayList<Timetable>)
        })

        // Set up the FAB to open the dialog
        view.findViewById<FloatingActionButton>(R.id.fabAddEntryFriday).setOnClickListener {
            showAddDataDialog()
        }
        return view
    }
